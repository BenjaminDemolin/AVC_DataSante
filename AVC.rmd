...
# Genre
t <- chisq.test(data_format$hypertension,data_format$stroke) #rejet de l'hypothese d'independance
t$residuals # les personness avec de l'hypertension ont plus de chance d'etre associees avec la survenue d'un AVC
t$p.value

# Maladie cardiaque
t <- chisq.test(data_format$heart_disease,data_format$stroke) #rejet de l'hypothese d'independance
t$residuals # les personnes avec des maladies cardiaques ont plus de chance d'etre associees avec la survenue d'un AVC
t$p.value
...
Creation d'une première variable motivée par l'étude exploratoire descriptive
```{r}
...
# test de dependance avec la variable cible
t <- chisq.test(AgeHypertension,avcQ$stroke)
t$residuals
#===> les personnes ayant de l'hypertension ont plus de risque de faire un AVC quelque soit leur age mais cette difference de risque est plus grande pour les personnes jeunes, les personnes plus agees ont de plus forts risques de faire un AVC qu'ils aient de l'hypertension ou non

# on ajoute cette nouvelle variable interaction et on lance une deuxieme AFCM
...
```{r}
#===> la modalite htP pour les plus de 30 ans, et dans une moindre mesure pour les moins de 30 ans, semble plus consistante avec le risque d'AVC que les autres modalites (htN pour les plus de 30 ans et htN pour les plus de 30 ans)

### Conclusion : on choisit de ne conserver que 4 modalités :
### 1) moins de 30 ans sans hypertension a0_30htN ; 2) moins de 30 ans avec hypertension a0_30htP ; 3) plus de 30 ans sans hypertension a30_plushtN ; 4) plus de 30 ans avec hypertension a30_plushtP

AgHt=as.factor(rep(c("a0_30htN","a0_30htP","a30_plushtN", "a30_plushtP"),90))
AgHt[avcQ2$AgeHypertension=="0_30:htN"]="a0_30htN" # jeunes sans hypertension
AgHt[avcQ2$AgeHypertension=="0_30:htP"]="a0_30htP" # jeunes avec hypertension
AgHt[avcQ2$AgeHypertension=="31_50:htN" | avcQ2$AgeHypertension=="51_65:htN" | avcQ2$AgeHypertension=="66_plus:htN"]="a30_plushtN" # plus de 30 ans sans hypertension
AgHt[avcQ2$AgeHypertension=="31_50:htP" | avcQ2$AgeHypertension=="51_65:htP" | avcQ2$AgeHypertension=="66_plus:htP"]="a30_plushtP" # plus de 30 ans avec hypertension
summary(AgHt)

avcQ3=data.frame(avcQ,"AgeHypertension"=AgHt)
summary(avcQ3)

### Finalement, 3e AFCM
quali3 <- avcQ3[,-c(1,9,10)]
var_sup3 <- avcQ3[,c("ageQ","stroke","hypertension")]
acm3 <- dudi.acm(quali3, scannf = FALSE, nf = 5)
acm3$supv <- supcol(acm3, dudi.acm(var_sup3, scannf = FALSE, nf = 5)$tab)

#explor(acm3)
###===> Les jeunes (moins de 30 ans) sont bien consistants avec un faible risque d'AVC
```

Creation d'une deuxième variable motivée par l'étude exploratoire descriptive
```{r}
# creation de la variable interaction entre age et maladie cardiaque
AgeCardiaque=avcQ$ageQ:avcQ$heart_disease
table(AgeCardiaque,avcQ[,"stroke"])

# test de dependance avec la variable cible
t <- chisq.test(AgeCardiaque,avcQ$stroke)
t$residuals
#===> les personnes ayant une maladie cardiaque ont plus de risque de faire un AVC mais cette tendance se reduit au-dela de 50 ans et meme s'inverse pour les plus de 65 ans, qui ont un risque eleve dans tous les cas mais plus fort en l'absence de maladie cardiaque

# on ajoute cette nouvelle variable interaction et on lance une deuxieme AFCM
avcQ2=data.frame(avcQ,"AgeCardiaque"=AgeCardiaque)
summary(avcQ2)
quali2 <- avcQ2[,-c(1,9,10)]
...
```{r}
#===> la modalite hdP pour les plus de 30 ans, et dans une moindre mesure pour les moins de 30 ans, semble plus consistante avec le risque d'AVC que les autres modalites (hdN pour les plus de 30 ans et hdN pour les plus de 30 ans)

### Conclusion : on choisit de ne conserver que 3 modalités :
### 1) moins de 30 ans sans maladie cardiaque a0_30hdN ; 2) plus de 30 ans sans maladie cardiaque a30_plushdN ; 3) tous ages avec maladie cardiaque a0_plushdP

AgMc=as.factor(rep(c("a0_30hdN","a30_plushdN","a0_plushdP"),90))
AgMc[avcQ2$AgeCardiaque=="0_30:hdN"]="a0_30hdN" # jeunes sans maladie cardiaque
AgMc[avcQ2$AgeCardiaque=="31_50:hdN" | avcQ2$AgeCardiaque=="51_65:hdN" | avcQ2$AgeCardiaque=="66_plus:hdN"]="a30_plushdN" # plus de 30 ans sans maladie cardiaque
AgMc[avcQ2$AgeCardiaque=="0_30:hdP" | avcQ2$AgeCardiaque=="31_50:hdP" | avcQ2$AgeCardiaque=="51_65:hdP" | avcQ2$AgeCardiaque=="66_plus:hdP"]="a0_plushdP" # tous ages avec maladie cardiaque
summary(AgMc)

avcQ3=data.frame(avcQ,"AgeCardiaque"=AgMc)
summary(avcQ3)

### Finalement, 3e AFCM
quali3 <- avcQ3[,-c(1,9,10)]
var_sup3 <- avcQ3[,c("ageQ","stroke","heart_disease")]
acm3 <- dudi.acm(quali3, scannf = FALSE, nf = 5)
acm3$supv <- supcol(acm3, dudi.acm(var_sup3, scannf = FALSE, nf = 5)$tab)

#explor(acm3)
###===> Les personnes plus agees (plus de 50 ans) sont bien consistants avec un plus fort risque d'AVC
```

Creation d'une troisième variable motivée par l'étude exploratoire descriptive
```{r}
# creation de la variable interaction entre age et hypertension
GlucHypertension=avcQ$glucoseQ:avcQ$hypertension
table(GlucHypertension,avcQ[,"stroke"])

# test de dependance avec la variable cible
t <- chisq.test(GlucHypertension,avcQ$stroke)
t$residuals
#===> les personnes ayant de l'hypertension ont plus de risque de faire un AVC quelque soit leur niveau de glucose mais le risque d'AVC augmente avec un niveau de glucose qui n'est pas entre 70 et 100

# on ajoute cette nouvelle variable interaction et on lance une deuxieme AFCM
avcQ2=data.frame(avcQ,"GlucHypertension"=GlucHypertension)
summary(avcQ2)
quali2 <- avcQ2[,-c(1,9,10)]
var_sup2 <- avcQ2[,c("glucoseQ","stroke","hypertension")]
acm2 <- dudi.acm(quali2, scannf = FALSE, nf = 5)
acm2$supv <- supcol(acm2, dudi.acm(var_sup2, scannf = FALSE, nf = 5)$tab)
...
```{r}
#===> les modalites gl127_plus avec ou sans hypertension et la hdP qu'importe le niveau de glucose semblent plus consistantes avec le risque d'AVC que les autres modalites (htN pour un niveau de glucose inferieur a 127)

### Conclusion : on choisit de ne conserver que 3 modalités :
### 1) glucose sous 127 sans hypertension gl0_127htN ; 2) glucose superieur a 127 sans hypertension gl127_plushtP ; 3) tous niveaux de glucose avec hypertension gl0_plushtP

GlHt=as.factor(rep(c("gl0_127htN","gl127_plushtN","gl0_plushtP"),90))
GlHt[avcQ2$GlucHypertension=="0_69:htN" | avcQ2$GlucHypertension=="70_100:htN" | avcQ2$GlucHypertension=="100_plus:htN"]="gl0_127htN" # glucose sous 127 sans hypertension
GlHt[avcQ2$GlucHypertension=="127_plus:htN"]="gl127_plushtN" # glucose superieur a 127 sans hypertension
GlHt[avcQ2$GlucHypertension=="0_69:htP" | avcQ2$GlucHypertension=="70_100:htP" | avcQ2$GlucHypertension=="101_126:htP" | avcQ2$GlucHypertension=="127_plus:htP"]="gl0_plushtP" # tous niveaux de glucose avec hypertension
summary(GlHt)

avcQ3=data.frame(avcQ,"GlucHypertension"=GlHt)
summary(avcQ3)

### Finalement, 3e AFCM
quali3 <- avcQ3[,-c(1,9,10)]
var_sup3 <- avcQ3[,c("glucoseQ","stroke","hypertension")]
acm3 <- dudi.acm(quali3, scannf = FALSE, nf = 5)
acm3$supv <- supcol(acm3, dudi.acm(var_sup3, scannf = FALSE, nf = 5)$tab)

#explor(acm3)
###===> L'hypertension, et dans une moindre mesure un fort taux de glucose, est bien consistante avec un fort risque d'AVC
```
